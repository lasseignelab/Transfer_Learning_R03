...
# START 
    ...
    We investigated the centrality of the highest weighted genes from all the latent variables and found that the top 10 highest weighted genes (aka. an important gene for the latent variable) for all the latent variables compared to genes that were not a top gene in any of latent gene were higher across all three centrality metrics 
...
```
there are genes that mostl yhave zero weights with an outlier 
...
looking at the string database for these top latent variable genes
```{r}
...
length(gene_list)
```
This line plot with the overlap between top genes (Number on the x-axis) and the unique function which describe the number of all the top genes from each LVs (removing repeats) over the total number of genes (without removing shared top genes). The vertical line is at 10 top genes and the horizontal line is at 0.80 were the cut offs for the top genes for this network analysis. 
...
ggplot(string_ppi_details, aes(x=top_10_weights, y=degree_log, fill=top_10_weights )) + 
  geom_violin()+ geom_point() +labs(title="LV PPI Degree",x="Top 10 Weight?", y = "PPI log(Degree + 1)", color= "Top 10 Weight?") + scale_colour_manual(values =  c("#440154FF","#228C8DFF"), aesthetics = c("colour", "fill"))+ theme_minimal()
```
```{r}
...
ggplot(string_ppi_details, aes(x=top_10_weights, y=log_betweness , fill=top_10_weights)) + 
  geom_violin()+ geom_point() +labs(title="LV PPI Betweeness",x="Top 10 Weight?", y = "PPI log(Betweeness + 1)", color= "Top 10 Weight?") + scale_colour_manual(values =  c("#440154FF","#228C8DFF"), aesthetics = c("colour", "fill"))+ theme_minimal()
```
```{r}
...
Conduct wilcox test to determine if there is a difference in network centrailtity betweeen genes that are higly weighted in teh latent variables vs not highly weighted.
```{r}
#
