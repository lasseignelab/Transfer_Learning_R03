...
```{r}
count_table<- readRDS( "~/data/recount3_GBM_GTEx_BRAIN_raw_counts.rds")
...
```
```{r}
knitr::include_graphics("~/output/limma_gbm/220421_ggplot_limma_tau_gbm_approved_candidates_pathways.png")
```
...
```{r eval= FALSE}
#note this requires the deseq2 output for GBM
...
for (i in 1:nrow(limma_sig_drugs_fda) ){
  targets<- as.vector(unlist(strsplit(limma_sig_drugs_fda$t_gn_sym[i], "; ")))
  #print(targets)
  drug_analysis( limma_sig_drugs_fda$pert[i], targets, as.data.frame(tpm_df) , all_metadata$status, Deseq2_gbm_normal_gtx_res, se= se_asssay, cell_lincs= "__GI1__trt_cp", "~/output/limma_gbm/220808_candidate_plots/")
}
```
