...
```{r}
recount3_wo_gbm_tpm <- recount3_tpm[, colnames(recount3_tpm) %in% human_samples_not_gbm]
```
...
```{r}
row_sd<- function (index, tpm_table){
  std <- sd(tpm_table[index,])
  return(std)
}

row_mean<- function(index, tpm_table){
  m<-  mean(as.numeric(tpm_table[index,]))
  return(m)
}
Row_Z_score <- function(index, tpm_table, m, std){
  #mean
  score<- tpm_table[index, ]
  m<- m[index]
  std<- std[index]
  #zscore
  Row_Z_score<- (score - m)/ std
  return(Row_Z_score)
}
```
...
```{r}
test_sd <- mclapply( 1:nrow(recount3_group1_tpm), row_sd, recount3_group1_tpm)
test_mean<- mclapply( 1:nrow(recount3_group1_tpm), row_mean, recount3_group1_tpm)
test_z_score<- mclapply( 1:nrow(recount3_group1_tpm), Row_Z_score, recount3_group1_tpm, test_mean, test_sd)
```
