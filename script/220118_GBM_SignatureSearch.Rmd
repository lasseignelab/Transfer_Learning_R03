...
```
Tau (τ) that ranges from -100 to +100 
...
ggsave("~/output/deseq2_gbm/220808_ggplot_deseq2_tau_gbm_candidates.pdf")
```
```{r}
write.csv(lincs_deseq_gbm_res_GI1_top_candidates, file = "~/output/deseq2_gbm/220427_SR_LINCS_GBM_DESEQ2_RES_all.csv")
```
...
```{r}
write.csv(hyperG_k_res_v2, file= "~/output/deseq2_gbm/220808_SR_LINCS_GBM_gbm_cell_drug_pathway_RES.csv" )
```
...
ggsave("~/output/deseq2_gbm/220808_ggplot_deseq2_tau_gbm_candidates_kegg_pathways.pdf")
```
...
```{r}
write.csv(hyperG_res_go, file= "~/output/deseq2_gbm/220808_SR_LINCS_GBM_gbm_cell_drug_Go_terms_RES.csv" )
```
...
for (i in 1:nrow( lincs_deseq_gbm_res_GI1_top_candidates) ){
  ...
  #print(targets)
  drug_analysis(  lincs_deseq_gbm_res_GI1_top_candidates$pert[i], targets, as.data.frame(tpm_df) , metadata$status, deseq_results, se= se_asssay, cell_lincs= "__GI1__trt_cp", "~/output/deseq2_gbm/220808_candidate_plots/")
}
```
