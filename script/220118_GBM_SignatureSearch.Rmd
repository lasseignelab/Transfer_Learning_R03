...
metadata <- readRDS("~/data/recount3_GBM_GTEx_BRAIN_metadata.rds")
```
...
66 / 66 genes in down set share identifiers with reference database
...
qsig_lincs_transfer_res <- qSig(query=list(upset=as.character(upset_v2), downset=as.character(downset_v2)), 
                   gess_method="LINCS", refdb=db_path)
```
52/ 52 genes in up set share identifiers with reference database
