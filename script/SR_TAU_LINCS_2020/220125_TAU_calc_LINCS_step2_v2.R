...
.libPaths(c("/data/user/jfisher7/R_Libs_4_1_0", "/data/rc/apps/rc/software/R/4.1.0-foss-2021a/lib64/R/library"))
...
f <- function(x, se, query_list, dest_dir) {
  ...
  write.table(as.data.frame(myMA), 
              file=paste0(dest_dir, "/result_", as.character(x)), 
              col.names=NA, quote=FALSE, sep="\t")
}

print("start parallel run")
#run through function 
f(num1, se=se, query_list=query_list, "/data/project/lasseigne_lab/JLF_scratch/Transfer_Learning_R03/data/LINCS_2020")
