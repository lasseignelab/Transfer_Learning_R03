...
query_list<- query_list[grep("GI1", names(query_list))]
...
f <- function(x, se, query_list, dest_dir) {
  ...
  chunkno <- x 
  sz <- 5 # small enough to use short queue 
  ...
  write.table(as.data.frame(myMA), 
              file=paste0(dest_dir, "/result_GBM_", as.character(x)), 
              col.names=NA, quote=FALSE, sep="\t")
}

print("start run")
#run through function 
f(69, se=se, query_list=query_list, "/data/project/lasseigne_lab/JLF_scratch/Transfer_Learning_R03/data/LINCS_2020")
