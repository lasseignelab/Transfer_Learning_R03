...
```{r eval= FALSE}
dds_luad_normal <- DESeqDataSetFromMatrix(countData = count_table,
...
deseq2_function <- function(dds, file_name, condition,  B, A){
  ...
  file_name <- paste0("~/output/lung_cancer/deseq2_res/",file_name, sep="")
...
```{r eval= FALSE}
deseq2_luad_normal <- deseq2_function(dds_luad_normal, "Deseq2_luad_normal_gtx_res.csv", "status", "Primary Tumor", "Solid Tissue Normal")
```
signaturesearch 
Genes
```{r}
...
```{r}
write.csv(lincs_deseq_luad_res, file= "~/output/lung_cancer/deseq2_res/220602_SR_LINCS_LUAD_DESEQ2_RES.csv" )
```
...
ggsave("~/output/lung_cancer/deseq2_res/220421_ggplot_deseq2_tau_luad_approved_candidates.png")
```

```{r}
#lincs_deseq_luad_res_A549_top 
write.csv(lincs_deseq_luad_res_A549_top , file= "~/output/lung_cancer/deseq2_res/220602_SR_LINCS_LUAD_DESEQ2_RES_tau.csv" )
```
...
```{r}
write.csv(hyperG_k_res_v2 , file= "~/output/lung_cancer/deseq2_res/220602_SR_LINCS_LUAD_DESEQ2_PATHWAY.csv" )
```
...
drug_analysis( "moxifloxacin", c("TOP2A"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/deseq2_res/220602_candidate_plot/")

drug_analysis( "zaleplon", c("GABRA1"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/deseq2_res/220602_candidate_plot/")

drug_analysis( "pseudoephedrine", c("SLC6A2", "SLC6A3", ),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/deseq2_res/220602_candidate_plot/")
...
filter by LUNG cancer cell lines
...
```{r}
primary_luad_res_sub <-primary_luad_res[ primary_luad_res$Var2 %in% lincs_deseq_luad_res_A549_top_fda$pert,]
```


```{r}
write.csv(primary_luad_res_sub , file= "~/output/lung_cancer/deseq2_res/220602_prism_deseq2_candidates.csv" )
```
...
ggsave("~/output/lung_cancer/deseq2_res/220602_ggplot_deseq2_candidates_prism_screen1.png")
```
```{r}
...
ggsave("~/output/lung_cancer/deseq2_res/220602_ggplot_deseq2_candidates_prism_screen1_sensitive_cell_lines.png")
```
...
```{r}
x<- 4/31
deseq2_CT_test_res <- clinical_trial_testing(101, "A549", 31, x , clinical_trial_LUAD)
```


```{r}
drug_list <- as.vector(unique(primary_luad_res$Var2))
drug_fraction<- c()
for (i in 1:length(drug_list)){
  cells <-primary_luad_res$Sensitive[primary_luad_res$Var2 %in% drug_list[i]]
  cells<- cells[!is.na(cells)]
  if( length(cells) < 12){
    print(drug_list[i])
  }
...
limma_gene_TCGA_BH <- LVTestWrapperJLF_v2(b.matrix=TPM , sample.info.df= sampleinfo, phenotype.col= "Tumor", blocking= database, file.lead= "220601_limma_TCGA_GTEX_T_vs_NT_lung_cancer", plot.dir= "~/output/limma_gbm/", results.dir= "~/output/lung_cancer/limma_res/", use.bonferroni= FALSE, sig.threshold = 0.05)
```


```{r eval=FALSE}
saveRDS(limma_gene_TCGA_BH , file= "~/output/lung_cancer/limma_res/220601_LUAD_GTEX_gene_limma_res.rds")
```
...
```{r}
write.csv(lincs_limma_lung_res, file= "~/output/lung_cancer/limma_res/220601_SR_LINCS_LUNG_LIMMA_RES.csv" )
```
...
```{r}
write.csv(lincs_limma_lung_res_A549, file= "~/output/lung_cancer/limma_res/220601_lincs_limma_lung_res_A549_tau.csv" )
```
...
ggsave("~/output/lung_cancer/limma_res/220421_ggplot_limma_tau_luad_approved_candidates_top_30.png")
```

```{r}
#lincs_deseq_luad_res_A549_top 
write.csv(lincs_limma_lung_res_A549_top , file= "~/output/lung_cancer/limma_res/220602_SR_LINCS_LUAD_limma_RES_tau.csv" )
```
...
```{r}
write.csv(hyperG_k_res_v2 , file= "~/output/lung_cancer/limma_res/220602_SR_LINCS_LUAD_limma_PATHWAY.csv" )
```
...
```{r}
#drug_analysis(drug_name, target_list,  tpm_df, sample_vector, deseq_results, se, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/deseq2_res/220602_candidate_plot/")

drug_analysis( "tigecycline", c("Y_RNA" ),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/limma_res/220602_candidate_plot/")
...
```{r}
primary_luad_res_sub <-primary_luad_res[ primary_luad_res$Var2 %in% lincs_limma_lung_res_A549_top_fda$pert,]
```


```{r}
write.csv(primary_luad_res_sub , file= "~/output/lung_cancer/limma_res/220602_prism_limma_candidates.csv" )
```

```{r}
primary_luad_res_sub <-primary_luad_res[ primary_luad_res$Var2 %in% lincs_limma_lung_res_A549_top_fda$pert[1:30],]
```
...
ggplot(primary_luad_res_sub, aes(x=log_fold_change, y= Var2))+ geom_violin() +geom_vline(xintercept = 0.3)  + ylab("Drug Candidates") + xlab("log fold change")
ggsave("~/output/lung_cancer/limma_res/220602_ggplot_limma_candidates_prism_screen1.png")
```
...
drug_senstive_precentage_all_drugs$drug_list<- factor(drug_senstive_precentage_all_drugs$drug_list, levels= unique(drug_senstive_precentage_all_drugs$drug_list[order(drug_senstive_precentage_all_drugs$drug_fraction)]))
ggplot(drug_senstive_precentage_all_drugs, aes( drug_fraction, drug_list, fill= test ))+ geom_bar(stat="identity") + scale_fill_viridis_d() + ylab("Drug Candidates") + xlab("Fraction of Sensitive Cell Lines") +labs(fill = "75% of Cell line are Sensitive")
ggsave("~/output/lung_cancer/limma_res/220602_ggplot_limma_candidates_prism_screen1_sensitive_cell_lines.png")
```
...
```{r}
table(lincs_limma_lung_res_A549_top_fda$clinical_trial_LUAD)
```


```{r}
limma_CT_test_res <- clinical_trial_testing(101, "A549", 85, 0.07058824, clinical_trial_LUAD)
```
...
```{r eval= FALSE}
recount3_plier_luad <- readRDS("~/data/recount3_plier_luad.rds")
```
```{r eval= FALSE}
...
test <- GetOrderedRowNorm(as.matrix(tpm_table),recount3_plier_luad )
...
create the b.matrix 
```{r eval= FALSE}
LUAD.GTEx.b.matrix <- GetNewDataB(as.matrix(tpm_table_v2), recount3_plier_luad)
```

```{r eval=FALSE}
saveRDS(LUAD.GTEx.b.matrix, file= "~/output/lung_cancer/TFL_res/LUAD_GTEX_B_MATRIX.rds")
```
...
LV_test_TCGA_BH <- LVTestWrapperJLF(b.matrix=LUAD.GTEx.b.matrix , sample.info.df= sampleinfo, phenotype.col= "Tumor", blocking= database, file.lead= "LV_TCGA_GTEX_T_vs_NT_LUAD", plot.dir= "~/output/lung_cancer/TFL_res/", results.dir= "~/output/lung_cancer/TFL_res/", use.bonferroni= FALSE, sig.threshold = 0.05)
```

```{r eval=FALSE}
saveRDS(LV_test_TCGA_BH , file= "~/output/lung_cancer/TFL_res/LUAD_GTEX_limma_TFL_res.rds")
```
```{r eval=FALSE}
saveRDS(all_metadata_v3 , file= "~/output/lung_cancer/TFL_res/LUAD_GTEX_metadata.rds")
```
...
```
```{r}
LUAD_GTEX_B_MATRIX <- readRDS("~/output/lung_cancer/TFL_res/LUAD_GTEX_B_MATRIX.rds")
```

```{r}
colSideColors<- ifelse(all_metadata_v3$status =="Primary Tumor","#440154FF","#228C8DFF" )
LUAD_GTEX_B_MATRIX_SIG<- LUAD_GTEX_B_MATRIX [rownames(LUAD_GTEX_B_MATRIX ) %in% SIG_LVs, ]
...
```{r}
sig_Z_mt <- recount3_plier_luad$Z[,LV_test_TCGA_BH$limma$adj.P.Val < 0.05 & abs(LV_test_TCGA_BH$limma$logFC)> .075 ]
...
```{r}
topgenes<- unique(c(topgene_1, topgene_2,topgene_3, topgene_4, topgene_5, topgene_6, topgene_7))
```
...
ggsave("~/output/lung_cancer/TFL_res/220421_ggplot_tfl_tau_luad_approved_candidates.png")
```

```{r}
#lincs_deseq_luad_res_A549_top 
write.csv(lincs_tfl_lung_res_A549_top , file= "~/output/lung_cancer/TFL_res/220602_SR_LINCS_LUAD_TFL_RES_tau.csv" )
```
...
```{r}
write.csv(hyperG_k_res_v2 , file= "~/output/lung_cancer/TFL_res/220602_SR_LINCS_LUAD_TFL_PATHWAY.csv" )
```
...
```{r}
primary_luad_res_sub <-primary_luad_res[ primary_luad_res$Var2 %in% lincs_tfl_lung_res_A549_top_fda$pert,]
```


```{r}
write.csv(primary_luad_res_sub , file= "~/output/lung_cancer/TFL_res/220602_prism_tfl_candidates.csv" )
```
...
primary_luad_res_sub$Var2 <- factor(primary_luad_res_sub$Var2, levels=unique(primary_luad_res_sub$Var2[order(primary_luad_res_sub$log_fold_change)]))
ggplot(primary_luad_res_sub, aes(x=log_fold_change, y= Var2))+ geom_violin() +geom_vline(xintercept = 0.3)  + ylab("Drug Candidates") + xlab("log fold change")
ggsave("~/output/lung_cancer/TFL_res/220602_ggplot_TFL_candidates_prism_screen1.png")
```
...
drug_senstive_precentage_all_drugs$drug_list<- factor(drug_senstive_precentage_all_drugs$drug_list, levels= unique(drug_senstive_precentage_all_drugs$drug_list[order(drug_senstive_precentage_all_drugs$drug_fraction)]))
ggplot(drug_senstive_precentage_all_drugs, aes( drug_fraction, drug_list, fill= test ))+ geom_bar(stat="identity") + scale_fill_viridis_d() + ylab("Drug Candidates") + xlab("Fraction of Sensitive Cell Lines") +labs(fill = "75% of Cell line are Sensitive")
ggsave("~/output/lung_cancer/TFL_res/220602_ggplot_TFL_candidates_prism_screen1_sensitive_cell_lines.png")
```
LINCS pathway & Drug targets
...
drug_analysis( "dabrafenib", c("BRAF" , "RAF1", "SIK1", "NEK11", "LIMK1"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "toremifene", c("ESR1" , "SHBG"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "trametinib", c("MAP2K1" , "MAP2K2"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "atorvastatin", c("HMGCR" , "DPP4", "AHR", "HDAC2", "NR1I3"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "sirolimus", c("MTOR"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "trifluoperazine", c("DRD2", "CALY", "ADRA1A", "CALM1", "TNNC1", "S100A4"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "selumetinib", c("MAP2K1", "MAP2K2"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "simvastatin", c("HMGCR", "ITGAL", "HDAC2"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "itraconazole", c("ERG11", "CYP51A1"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")

drug_analysis( "erlotinib", c("NR1I2", "EGFR"),  tpm_df, all_metadata_v3$status , deseq2_luad_normal, se= se_asssay, cell_lincs= "__A549__trt_cp", "~/output/lung_cancer/TFL_res/220602_candidate_plot")
```
...
```{r}
table(lincs_tfl_lung_res_A549_top_fda$clinical_trial_LUAD)
```

```{r}
tfl_CT_test_res <- clinical_trial_testing(101, "A549", 81, 0.1975309, clinical_trial_LUAD)
```
...
```{r}
venn_dia_methods(lincs_limma_gbm_res_A549_fda_approved$pert, lincs_deseq_gbm_res_A549_fda_approved$pert, lincs_tfl_gbm_res_A549_fda_approved$pert, file_name='~/output/lung_cancer/SR_drug_candidates_venn_diagram.png' )
```
