...
```{r}
id_list <-recount3_rse_PAAD@colData$tcga.tcga_barcode[ grep("NEUROENDOCRINE",recount3_rse_PAAD@colData$tcga.cgc_case_other_histological_diagnosis, ignore.case = TRUE)]
...
```{r}
write.csv(lincs_deseq_paad_res, file= "~/output/pancreas_cancer/deseq2_res/220808_SR_LINCS_PAAD_DESEQ2_RES.csv" )
```
...
write.csv(lincs_deseq_paad_res_YAPC_top , file= "~/output/pancreas_cancer/deseq2_res/220808_SR_LINCS_PAAD_DESEQ2_RES_tau.csv" )
```
...
```{r}
write.csv(hyperG_k_res_v2 , file= "~/output/pancreas_cancer/deseq2_res/220808_SR_LINCS_PAAD_DESEQ2_PATHWAY.csv" )
```
...
for (i in 1:nrow(lincs_deseq_paad_res_YAPC_top_fda) ){
  ...
  #print(targets)
  try(drug_analysis( lincs_deseq_paad_res_YAPC_top_fda$pert[i], targets, tpm_df , all_metadata_v3$status, deseq2_paad_normal, se= se_asssay, cell_lincs= "__YAPC__trt_cp", "~/output/pancreas_cancer/deseq2_res/candidate_plots"))
}
```
...
```{r}
downset<- deseq2_paad_tfl$Symbol[deseq2_paad_tfl[,2] < 0]
upset<- deseq2_paad_tfl$Symbol[deseq2_paad_tfl[,2] > 0]
```
