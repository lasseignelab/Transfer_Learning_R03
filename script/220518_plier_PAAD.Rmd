...
```{r}
human_samples_not_paad<- human_samples$external_id[! human_samples$project == "PAAD" | human_samples$project  == "PANCREAS"]
```

```{r}
library(dplyr)
recount3_wo_paad_tpm <- recount3_tpm_v2[, colnames(recount3_tpm_v2) %in% human_samples_not_paad]
```

```{r}
rm(human_samples, human_samples_not_paad, recount3_tpm, recount3_tpm_v2)
gc()
```


```{r}
recount3_wo_paad_tpm<- as.matrix(recount3_wo_paad_tpm)
rownames(recount3_wo_paad_tpm) <- gene_info$gene_name
```
...
```{r}
recount3_plier_paad <- PLIERNewData_JLF(recount3_wo_paad_tpm)
saveRDS(recount3_plier_paad, "/data/project/lasseigne_lab/JLF_scratch/Transfer_Learning_R03/data/recount3/recount3_plier_paad.rds")
```
