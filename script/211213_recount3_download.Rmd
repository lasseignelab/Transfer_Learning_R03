...
# START 
    ...
    Donwloading Recount 3 RNA-seq Counts and normalizing counts by gene length via tpm 
...
location on UAB Cheaha cluster 
...
library(recount3)
```
collect human project project ids. 
```{r eval= FALSE}
human_projects<- available_projects()
```

#GBM
download the GBM and normal brain data from recount3 
```{r eval= FALSE}
 human_recount_projects_gbm <- human_projects[! (human_projects$project == "GBM" | human_projects$project == "BRAIN"),]
```
downloading the rse with the gene expression data
```{r eval= FALSE}
test<- create_rse(human_recount_projects_gbm[1,])
recount3__gbm_tf <- as.data.frame(assay(test))
```

Using apply to find all the gene expression url locations for downloading
```{r eval= FALSE}
...
```
create the wget commands to download the data 
```{r eval= FALSE}
...
```
creating the sh files for UAB cheaha to download via the slurm queue system
```{r eval= FALSE}
...
after the data is download via the sh scripts. This function was used to convert files in one directory to count table in R. 
```{r eval= FALSE}
...
Applying the count_table to all the directories with the recount3 files. 
```{r eval= FALSE}
...
gc()
```
read the rds object created from count_table function into R 
```{r eval= FALSE}
...
downloaded a random project to gene lengths for tpm calculations
```{r eval= FALSE}
#SRP118922
...
Apply tpm function to all the recount3 data
```{r eval= FALSE}
...
there was no reason for the removal of the first col from the matrix adding it back as group 9 done 220728 
```{r eval= FALSE}
